...
/*
  ensureUserRecord — يسجّل المستخدم في جدول users عند أول دخول
  هكذا الأونر يقدر يشوف UUID الكل في قاعدة البيانات
*/
...
async function submitServer() {
  ...
  const invite = $('fInvite').value.trim();
  const icon   = $('fIcon').value.trim();

  if (!name || !desc || !cat || !invite) { toast('⚠️ أكمل الحقول المطلوبة', 'error'); return; }
  if (!invite.startsWith('https://discord.gg/') && !invite.startsWith('https://discord.com/invite/')) {
    toast('⚠️ رابط الدعوة يجب أن يبدأ بـ https://discord.gg/', 'error'); return;
  }
  const { error } = await sb.from('servers').insert({ name, description: desc, category: cat, invite_url: invite, icon_url: icon || null, owner_id: user.id, approved: false });
